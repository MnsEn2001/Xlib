...
-- Remove existing ShiftLockScreenGui if it exists
local existingGui = game.CoreGui:FindFirstChild("Shiftlock (CoreGui)")
if existingGui then
    existingGui:Destroy()
end
...
-- New function to delete button after holding for 3 seconds
local function onButtonHeld(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        local holdStart = tick()

        -- Wait for 3 seconds while holding down the button
        while input.UserInputState == Enum.UserInputState.Begin and (tick() - holdStart < 3) do
            wait()
        end

        -- Check if held for 3 seconds
        if tick() - holdStart >= 3 then
            ShiftLockButton:Destroy() -- Destroy the button if held for 3 seconds
        end
    end
end

ShiftLockButton.InputBegan:Connect(onButtonHeld) -- Connect the hold event to the button
